...
<div class="max-w-6xl mx-auto pb-10">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
        <div>
            <div class="flex items-center gap-2 flex-wrap">
                <h1 class="text-2xl font-bold text-blue-600 tracking-tight">{{ $server->name }}</h1>
                <span class="badge {{ $server->status === 'Online' ? 'status-online' : ($server->status === 'Offline' ? 'status-offline' : 'status-warning') }}">
                    {{ $server->status ?? 'Unknown' }}
                </span>
            </div>
            <p class="text-sm text-gray-500 mt-1">Detail lengkap informasi dan konfigurasi server</p>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-4">
        <div class="lg:col-span-2 bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
            ...
            <dl class="divide-y divide-gray-100 text-sm">
                <div class="flex flex-col sm:flex-row sm:gap-6 py-2">
                    <dt class="sm:w-56 shrink-0 text-gray-500">Nama Server</dt>
                    <dd class="font-mono font-medium text-gray-900">{{ $server->name }}</dd>
                </div>
                <div class="flex flex-col sm:flex-row sm:gap-6 py-2">
                    <dt class="sm:w-56 shrink-0 text-gray-500">Sistem Operasi (OS)</dt>
                    <dd class="text-gray-900">{{ $server->os ?? '-' }}</dd>
                </div>
                <div class="flex flex-col sm:flex-row sm:gap-6 py-2">
                    <dt class="sm:w-56 shrink-0 text-gray-500">Versi OS</dt>
                    <dd class="text-gray-900">{{ $server->os_version ?? '-' }}</dd>
                </div>
                <div class="flex flex-col sm:flex-row sm:gap-6 py-2">
                    <dt class="sm:w-56 shrink-0 text-gray-500">Tipe Server</dt>
                    <dd class="text-gray-900">{{ $server->type ?? '-' }}</dd>
                </div>
                <div class="flex flex-col sm:flex-row sm:gap-6 py-2">
                    <dt class="sm:w-56 shrink-0 text-gray-500">Jenis (Kind)</dt>
                    <dd>
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-5 shadow-sm flex flex-col">
            <h3 class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">Informasi Waktu</h3>
            <div class="flex-1 flex flex-col gap-4 text-sm">
                <div>
                    <span class="block text-xs text-gray-400 mb-1">Dibuat Pada</span>
                    <span class="font-medium text-gray-900">{{ $server->created_at ? $server->created_at->format('d M Y, H:i') : '-' }}</span>
                </div>
                <div>
                    <span class="block text-xs text-gray-400 mb-1">Terakhir Diperbarui</span>
                    <span class="font-medium text-gray-900">{{ $server->updated_at ? $server->updated_at->format('d M Y, H:i') : '-' }}</span>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-5 shadow-sm mb-4">
        ...
        <dl class="divide-y divide-gray-100 text-sm">
            <div class="flex flex-col sm:flex-row sm:gap-6 py-2">
                <dt class="sm:w-56 shrink-0 text-gray-500">IP Utama (Primary)</dt>
                <dd class="text-gray-900">
                    @if($primaryIp)
                        <span class="inline-flex items-center font-mono text-sm px-2.5 py-1 rounded bg-blue-100 text-blue-700 border border-blue-200" title="Tipe: {{ $primaryIp->type }}">
            ...
            <div class="flex flex-col sm:flex-row sm:gap-6 py-2">
                <dt class="sm:w-56 shrink-0 text-gray-500">IP Lainnya</dt>
                <dd class="text-gray-900">
                    @if($otherIps->isEmpty())
                        <span class="text-gray-400">-</span>
                    @else
    ...
    {{-- ============================================= --}}
    {{-- CATATAN TAMBAHAN (Opsional, jika ada kolom description/notes) --}}
    {{-- ============================================= --}}
    @if(isset($server->description) || isset($server->notes))
    <div class="bg-white rounded-xl border border-gray-200 p-5 shadow-sm mb-4">
        <h3 class="flex items-center gap-2 text-xs font-semibold text-blue-700 uppercase tracking-wider mb-4 pb-3 border-b border-gray-100">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
