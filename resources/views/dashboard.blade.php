...
<div x-data="{
    ...
    matchesServer(name, ip, ips, os, type, kind){
        const q = this.serverSearch.trim().toLowerCase();
        const matchSearch = !q || name.toLowerCase().includes(q) || ip.toLowerCase().includes(q) || (ips && ips.some(i => i.toLowerCase().includes(q)));
    ...
    matchesSubdomain(sub, domain, status, ip, ips){
        const q = this.subdomainSearch.trim().toLowerCase();
        const matchSearch = !q || sub.toLowerCase().includes(q) || domain.toLowerCase().includes(q) || ip.toLowerCase().includes(q) || (ips && ips.some(i => i.toLowerCase().includes(q)));
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse($servers as $server)
                            @php
                                ...
                                $ipText    = $primaryIp ? $primaryIp->ip_address : '';
                ...
                <table class="w-full text-xs">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse($subdomains as $subdomain)
                            @php
                                $subIpText = $subdomain->ips->isNotEmpty()
                                    ? $subdomain->ips->first()->ip_address
                                    : '';
    ...
    <script>
        ...
        function createLayeredGradient(ctx, chartArea, hex, steps = 22) {
            ...
            for (let i = 0; i < steps; i++) {
                const t = i / (steps - 1);
                gradient.addColorStop(i / steps, mixWithWhite(hex, 0.72 * (1 - t)));
                gradient.addColorStop(Math.min((i + 1) / steps - 0.001, 1), mixWithWhite(hex, 0.72 * (1 - t)));
            }
            return gradient;
        }
        ...
        const shadowPlugin = {
            ...
            beforeDraw: function(chart) {
                ...
                const arc = chart.getDatasetMeta(0).data[activeIndex];
                if (!arc) return;
                ctx.save();
                ctx.shadowColor = 'rgba(0, 0, 0, 0.3)';
                ctx.shadowBlur = 15; ctx.shadowOffsetX = 0; ctx.shadowOffsetY = 5;
                ctx.beginPath();
                ctx.arc(arc.x, arc.y, arc.outerRadius, arc.startAngle, arc.endAngle);
                ctx.arc(arc.x, arc.y, arc.innerRadius, arc.endAngle, arc.startAngle, true);
                ctx.closePath();
                ctx.fillStyle = createLayeredGradient(ctx, chartArea, osColorKeys[activeIndex], 22);
                ctx.fill();
                ctx.restore();
            }
        };
            ...
            data: {
                ...
                datasets: [{
                    ...
                    backgroundColor: function(context) {
                        ...
                        return createLayeredGradient(context.chart.ctx, context.chart.chartArea, osColorKeys[context.dataIndex], 22);
                    },
                    borderWidth: 2, borderColor: '#ffffff', hoverOffset: 12, hoverBorderWidth: 2, hoverBorderColor: '#ffffff'
                }]
            },
            options: {
                ...
                hover: {
                    ...
                    onHover: function(e, activeElements) {
                        if (activeElements.length > 0) {
                            ...
                            this.data.datasets[0].offset = this.data.datasets[0].data.map(() => 0);
                            this.data.datasets[0].offset[activeIndex] = 12;
                        } else {
                            activeIndex = null;
                            this.data.datasets[0].offset = this.data.datasets[0].data.map(() => 0);
                        }
