...
@section('title', 'Detail Aset - Data & Informasi')
...
<div class="max-w-6xl mx-auto">
    ...
    <div class="bg-white rounded-lg border border-gray-200 p-5 shadow-md mb-4">
        <h3 class="text-xs font-semibold text-blue-600 uppercase tracking-wider mb-3 border-b pb-2">Detail Data & Informasi</h3>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs">
            <div class="border-b border-gray-100 pb-2 flex justify-between"><span class="text-gray-500">Nomor Dokumen</span><span class="font-mono text-gray-900">{{ $asset->document_number ?? '-' }}</span></div>
            <div class="border-b border-gray-100 pb-2 flex justify-between"><span class="text-gray-500">Tahun Penyusunan</span><span class="text-gray-900">{{ $asset->year ?? '-' }}</span></div>
            <div class="border-b border-gray-100 pb-2 flex justify-between"><span class="text-gray-500">Status Aset</span><span class="px-1.5 py-0.5 rounded {{ $asset->status === 'Sudah Disahkan' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700' }}">{{ $asset->status ?? '-' }}</span></div>
            ...
            <div class="border-b border-gray-100 pb-2 flex justify-between"><span class="text-gray-500">Format Penyimpanan</span><span class="text-gray-900">{{ $asset->storage_format ?? '-' }}</span></div>
            ...
            <div class="border-b border-gray-100 pb-2 flex justify-between"><span class="text-gray-500">Retensi Aset</span><span class="text-gray-900">{{ $asset->retention ?? '-' }}</span></div>
            <div class="border-b border-gray-100 pb-2 flex justify-between"><span class="text-gray-500">Kerahasiaan</span><span class="text-gray-900">{{ $asset->confidentiality ?? '-' }}</span></div>
            <div class="border-b border-gray-100 pb-2 flex justify-between"><span class="text-gray-500">Integritas</span><span class="text-gray-900">{{ $asset->integrity ?? '-' }}</span></div>
            <div class="pb-2 flex justify-between"><span class="text-gray-500">Ketersediaan</span><span class="text-gray-900">{{ $asset->availability ?? '-' }}</span></div>
        </div>
    </div>
</div>
@endsection