...
<div class="flex items-center justify-between mb-4">
    ...
    <a href="{{ route('assets.create', ['category' => $activeCode]) }}" class="flex items-center gap-1.5 bg-blue-600 text-white px-4 h-9 rounded text-sm hover:bg-blue-700 shadow-sm shadow-blue-300 transition-colors">
        ...
        <table class="w-full text-xs whitespace-nowrap">
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                <tr class="hover:bg-gray-50 transition-colors">
                    ...
                    <td class="px-3 py-2.5 text-gray-600">{{ $asset->document_number ?? '-' }}</td>
                    <td class="px-3 py-2.5 text-gray-600">{{ $asset->year ?? '-' }}</td>
                    <td class="px-3 py-2.5">
                        <span class="badge {{ $asset->status === 'Sudah Disahkan' ? 'status-active' : 'status-warning' }}">
                            {{ $asset->status ?? '-' }}
                        </span>
                    </td>
                    <td class="px-3 py-2.5 text-gray-600">{{ $asset->location ?? '-' }}</td>
                    <td class="px-3 py-2.5 text-gray-600">{{ $asset->storage_format ?? '-' }}</td>
                    <td class="px-3 py-2.5 text-gray-600">{{ $asset->owner ?? '-' }}</td>
                    <td class="px-3 py-2.5 text-gray-600">{{ $asset->retention ?? '-' }}</td>
                    <td class="px-3 py-2.5 text-gray-600">{{ $asset->confidentiality ?? '-' }}</td>
                    <td class="px-3 py-2.5 text-gray-600">{{ $asset->integrity ?? '-' }}</td>
                    <td class="px-3 py-2.5 text-gray-600">{{ $asset->availability ?? '-' }}</td>
                    <td class="px-3 py-2.5">
                ...
                <tr>
                    <td colspan="15" class="px-4 py-8 text-center text-gray-500">
                        Belum ada data {{ $pageTitle }}. <a href="{{ route('assets.create', ['category' => $activeCode]) }}" class="text-blue-600 hover:underline">Tambah sekarang</a>
                    </td>
                </tr>
                @endforelse
            </tbody>
