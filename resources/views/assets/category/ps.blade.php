...
<div class="flex items-center justify-between mb-4">
    ...
    <a href="{{ route('assets.create', ['category' => 'PS']) }}" class="flex items-center gap-1.5 bg-blue-600 text-white px-4 h-9 rounded text-sm hover:bg-blue-700 shadow-sm shadow-blue-300 transition-colors">
...
<div x-data="{
    ...
    matches(code, name, nip, unit, position) {
        const q = (this.search || '').trim().toLowerCase();
        if (q === '') return true;
        return (code || '').toString().toLowerCase().includes(q)
            || (name || '').toString().toLowerCase().includes(q)
            || (nip || '').toString().toLowerCase().includes(q)
            || (unit || '').toString().toLowerCase().includes(q)
            || (position || '').toString().toLowerCase().includes(q);
    },
...
<div class="bg-white rounded-lg border border-gray-200 shadow-lg shadow-blue-500/10">
    ...
    <div class="p-4 border-b border-gray-200 bg-gray-50">
        <div class="flex items-center gap-2">
            <input type="text" x-model="search" placeholder="Cari nama, NIP/NIK, unit, jabatan..."
        ...
        <table class="w-full text-xs whitespace-nowrap">
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                <tr class="hover:bg-gray-50 transition-colors"
                    x-show="matches(@js($asset->asset_code), @js($asset->name), @js($asset->nip), @js($asset->unit), @js($asset->position))">
                    ...
                    <td class="px-3 py-2.5">
                        @if($asset->personnel_category === 'ASN')
                            <span class="badge badge-physical">ASN</span>
                        @elseif($asset->personnel_category === 'Pihak Ketiga')
                            <span class="badge badge-virtual">Pihak Ketiga</span>
                        @else
                            <span class="text-gray-500">{{ $asset->personnel_category ?? '-' }}</span>
                        @endif
                    </td>
                    <td class="px-3 py-2.5 font-mono text-gray-600">{{ $asset->nip ?? '-' }}</td>
                    <td class="px-3 py-2.5 text-gray-600">{{ $asset->function ?? '-' }}</td>
                    <td class="px-3 py-2.5 text-gray-600">{{ $asset->unit ?? '-' }}</td>
                    <td class="px-3 py-2.5 text-gray-600">{{ $asset->position ?? '-' }}</td>
